...
ValueEncodings = {'boolean', 'float32', 'float64', 'int', 'int8', 'int16', 'int32', 'int64', 'opaque', 'string',
                  'uint8', 'uint16', 'uint32', 'uint64'}
DataLevels = {'L0', 'L1', 'L2'}
DataProductTypes = {'Auxiliary Data', 'Data Product Type', 'Engineering Data', 'Enineering Data', 'Science Data',
                    'Unprocessed Data'}
...
class TestParameter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # hid - every human-readable identifier must be unique
        # hid conflict - must be 0 for all parameter rows in the database/table
        # parameter type - must be one of {array<*>, binary, boolean, category<*>, constant<*>, external, function,
        # quantity, record<>, }
        # value encoding - must be one of {boolean, [u]int{8,16,32,64}, float{32,64}, opaque}
        # code set - must be present if parameter type was category and contain enumeration string
        # unit of measure - TODO - supposed to match udunits symbol - c.f. udunits2-accepted.xml
        # fill value - if number, must be able to fit within size specified by value encoding
        # display name - enforce Title Case
        # precision - 0 is default, must be 0 or another integer value
        # visible - FALSE or TRUE or <empty>
        # parameter function id - if present, must be unique
        # parameter function map - must be defined if parameter function id is present
        # - must be a valid python dictionary
        # - each value must be valid parameter (if starting with PD)
        # lookup value - ignore - not used
        # qc functions - enforce ALL CAPS, ALPHA and hyphens only
        # standard name - must match to CF standard name table
        ...
        # data product identifier - must match SAF - TODO
        ...
        # long name - ignore
        # skip - ignore
        # data product type - must be one of {Auxiliary Data, Data Product Type, Engineering Data, Enineering Data,
        # Science Data, Unprocessed Data}
...
InstrumentClasses = {
    'All', 'ADCP', 'ADCPA', 'ADCPS', 'ADCPT', 'BOTPT', 'CTDBP', 'CTDGV', 'CTDMO', 'CTDPF', 'DOFST', 'DOSTA', 'FDCHP',
    'FLORD', 'FLORT', 'HYDBB', 'HYDLF', 'METBK', 'NUTNR', 'OBSBB', 'OBSSP', 'OPTAA', 'PARAD', 'PCO2A', 'PCO2W',
    'PHSEN', 'PRESF', 'PREST', 'SPKIR', 'THSPH', 'TRHPH', 'VADCP', 'VEL3D', 'VELPT', 'WAVSS'
}

FunctionTypes = {
    'NumexprFunction', 'PythonFunction', 'QCPythonFunction'
}
...
class TestFunctions(unittest.TestCase):
    ...
    # ID - must be unique
    # HID - must be unique - must be equal to <Name>_<InstClass>_<InstSer>
    # Name - must not contain spaces
    # Instrument Class - should be from the list of instrument classes - may be comma separated
    # Instrument Series - enforce alpha separated by commas
    # Function Type - must be from the list - one case has or - not sure if that is valid...
    ...
    # Owner - must be present if Function Type is PythonFunction (could verify that the function is available
...
DeliveryMethods = { 'Recovered', 'Streamed', 'Telemetered' }
DataTypes = { 'Science', 'Engineering', 'Calibration' }
...
class TestStream(unittest.TestCase):
    ...
    # parameter_ids - comma separated, parameter ids, must exist
    # temporal_parameter - must be set (default is PD7, PD3655, PD3660 or PD3074)
    ...
    # data level - must be one of {L0, L1, L2} - if present, should match the end of data product identifier
